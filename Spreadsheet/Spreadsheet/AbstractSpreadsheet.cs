...
namespace SS
{
    ...
    public abstract class AbstractSpreadsheet
    {
        ...
        /// 
        /// Gets the direct and indirect dependents of the Cell whose name is name, using Graphs, where the it tries to detect a cycle using DFS
        /// </summary>
        private void Visit(String start, String name, ISet<String> visited, LinkedList<String> changed)
        {
            //Adds the name of the cell to the set of visited cells
            visited.Add(name);

            //A loop that enumerates thru the values of the getDirectDependents of the name of the cell provided 
            foreach (String n in GetDirectDependents(name))
            {
                //If the dependent is equal to the starting cell, then we throw a CircularException because the graph has a cyclic
                if (n.Equals(start))
                {
                    throw new CircularException();
                }
                //Else if the dependent has not been visited yet, we go into this condition 
                else if (!visited.Contains(n))
                {
                    //We go into the Visit method(i.e perform recursion) and then try to get the direct dependents of n, perform the same 
                    // steps with n being the name parameter for the method
                    Visit(start, n, visited, changed);
                }
            }
            //Adds the name to the front of the changed linked list after getting all dependents
